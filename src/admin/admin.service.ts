...
export class AdminService {
 ...
 async updateCourse(id: string, updateData: any) {
    ...
    // Jika ada update pada author/instructor
    if (updateData.instructorId || updateData.authorId) {
      const targetAuthorId = updateData.instructorId || updateData.authorId;
      const author = await this.prisma.user.findUnique({
    ...
    const { instructorId, ...otherData } = updateData;

    
    const dataToUpdate = {
      ...otherData,
    
      ...(instructorId && { authorId: instructorId }) 
    };
